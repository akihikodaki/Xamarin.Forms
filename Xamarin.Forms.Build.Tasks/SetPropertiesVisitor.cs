...
namespace Xamarin.Forms.Build.Tasks
{
	class SetPropertiesVisitor : IXamlNodeVisitor
	{
		class Borrower
		{
			readonly ILContext context;
			readonly INode node;

			readonly Dictionary<string, (IEnumerable<Instruction>, FieldDefinition)> dictionary =
			...
			{
				if (dictionary.TryGetValue(name, out var cached))
				{
				...
				var variable = context.FindByName(name, node);
				if (variable != null)
			...
			private (TypeReference, IEnumerable<Instruction>) ReserveByInstructions(string name, TypeReference type, IEnumerable<Instruction> instructions)
			{
				...
				dictionary.Add(name, (instructions, field));
			...
			{
				foreach (var named in dictionary.Values)
				{
